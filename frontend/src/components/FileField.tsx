...
export function FileField({ label, registration, error }: FileFieldProps) {
  ...
  return (
    <label className="grid gap-2 text-sm font-medium text-[#4c4033]" htmlFor={inputId}>
      ...
      >
        <ImagePlus className={error ? 'text-[#b31316]' : 'text-[#8a5f13]'} size={24} aria-hidden="true" />
        <span className="text-sm font-semibold text-[#3f2c12]">Seleccionar foto</span>
        <span className="text-xs font-normal text-[#6f6a60]">JPG, PNG, WebP o GIF. Máximo 5 MB.</span>
      </span>
      <input id={inputId} type="file" accept="image/*" {...registration} className="sr-only" />
